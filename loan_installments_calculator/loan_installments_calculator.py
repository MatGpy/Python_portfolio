...
def takeUserDate(daysReversed, monthsReversed, loanAmount, loanCommission, loanInterestRate, periodOfPayment):
    userDate = str(input("Insert the date when your loan was taken (in dd-mm-yyyy format): "))
    userDay = userDate[:2]
    userMonth = userDate[3:5]
    userYear = userDate[6:]
    if userDay not in daysReversed or userMonth not in monthsReversed or int(userYear) <= 0 or loanAmount <= 0 or loanCommission <= 0 or loanInterestRate <= 0 or periodOfPayment <= 0:
        raise ValueError
...
def main():
    try:
        ...
        userDay, userMonth, userYear = takeUserDate(daysReversed, monthsReversed, loanAmount, loanCommission, loanInterestRate, periodOfPayment)
        ...
        print("Error, amount of your loan can't be lower than 1000 PLN and period of your loan's payment can't be longer than 20 years.")
